...
auto createPipeline() {
	...
	// Setup all data needed for the pipeline-tasks
...
bool minesweeperTest_solve(const std::string& processName) {
	...
	while (true) {
		...
		// 4. Process print and create grid
		GridData gridData(print);
		// These values should ideally be found using the image-processor, but need better ImageMatcher (currently per-pixel checks)
		gridData.tileWidth = tileSize;
		gridData.tileHeight = tileSize;
		auto grid = pipeline->process(gridData);

		// (just determining current state)
		if (gridData.isGameLost) {
			...
			// 5. Solve for next move
			auto sweeper = Sweeper();
			auto next = sweeper.getNextMove(*grid);

			// 6. Perform next move (this should definitly be encapsulated in some Command-design, but just had to get it working asap...
